...
class UserDialog extends StatelessWidget {
  ...
  final void Function(String roomId)? onStartConversation;
                          ...
                          child: RichText(
                            text: TextSpan(
                              children: [
                                ...
                                TextSpan(text: '@${profile.userId.localpart}'),
                              ],
      ...
      actions: [
        if (client.userID != profile.userId) ...[
          AdaptiveDialogAction(
            bigButtons: true,
            onPressed: () async {
              // Save context reference before dismissing dialog
              final navigator = Navigator.of(context);
              final router = GoRouter.of(context);
              final scaffoldContext = context; // Save for loading dialog

              // Dismiss dialog
              navigator.pop();

              // If room already exists, go directly
              if (dmRoomId != null) {
              ...
              // Create connection and then navigate to the new room
              client.startDirectChat(profile.userId).then((roomId) {
                navigator.pop(); // Dismiss loading using saved navigator

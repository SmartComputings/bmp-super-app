...
import 'package:bmp/shared/data/datasources/check_username_local_datasource.dart';
import 'package:bmp/shared/utils/push_helper.dart';
...
class BackgroundPush {
  ...
  void _handleFcmMessage(Map<dynamic, dynamic> message) async {
    await _initCompleter.future;
    Logs().i('[FCM] Received message: $message');
    
    try {
      final data = message['data'] as Map<String, dynamic>? ?? {};
      final notification = message['notification'] as Map<String, dynamic>?;
      
      // Handle Bridge/PayCrypto customer events - clear cache to force refresh
      final eventType = data['event_type'] as String?;
      if (eventType != null) {
        if (eventType == 'customer.created' || 
            eventType == 'customer.updated' ||
            eventType.startsWith('customer.')) {
          Logs().i('[FCM] Bridge event detected: $eventType - clearing check-username cache');
          await CheckUsernameLocalDataSourceImpl.clearAllCacheStatic();
        }
      }
      
      if (notification != null && !data.containsKey('room_id')) {
        Logs().i('[FCM] Notification with no room_id - skipping in BackgroundPush to avoid duplicates with NotificationService');
        // NotificationService handles general app notifications (OPEN_HOME, etc.) via FirebaseMessaging.onMessage
        return;
      }
      ...
      Logs().e('[FCM] Push helper error', e, s);
    }
  }
  ...
  void _init() async {
    try {
      ...
      firebase?.setListeners(
        ...
        onNewToken: (token) {
          _fcmToken = token;
          Logs().i('[FCM] Token refreshed: $token');
        },
      );
      if (Platform.isAndroid) {
  ...
  Future<void> setupFirebase() async {
    ...
    if (_fcmToken?.isEmpty ?? true) {
      try {
        ...
        Logs().i('[FCM DEBUG] Device Token: $_fcmToken');
      ...
      // Debug print existing FCM token
      Logs().i('[FCM DEBUG] Using existing token: $_fcmToken');
      print('=== EXISTING FCM TOKEN ===');
      print(_fcmToken);
      print('===========================');
    }
    await setupPusher(
      ...
      if (client.getRoomById(roomId) == null) {
        await client
            ...
            .timeout(
              ...
              onTimeout: () {
                Logs().w('[Push] Room sync timeout for $roomId');

  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.waiting && searchQuery.isNotEmpty) {
          return AppLoadingWidget();
        }

        final events = snapshot.data?.$1 ?? [];

        if (events.isEmpty && snapshot.connectionState == ConnectionState.done && searchQuery.isNotEmpty) {
          return Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [

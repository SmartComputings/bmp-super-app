...
import 'package:bmp/shared/utils/markdown_context_builder.dart';
...
class InputBar extends StatelessWidget {
  ...
  List<Map<String, String?>> getSuggestions(String text) {
    ...
    // Room suggestions disabled to hide server domains
  ...
  Widget buildSuggestion(
    ...
    if (suggestion['type'] == 'user' || suggestion['type'] == 'room') {
      ...
      final displayName = suggestion.tryGet<String>('displayname');
      final mxid = suggestion.tryGet<String>('mxid');
      final textToShow = displayName ??
          ((mxid != null && mxid.contains(':'))
              ? mxid.split(':').first.replaceAll(RegExp(r'^[!@#]'), '')
              : (mxid ?? ''));
      return Container(
        padding: padding,
        child: Row(
